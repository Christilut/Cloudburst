...
class MKVTorrent(Torrent.Torrent): # inherit from Torrent
    ...
    bufferSize = 3 # in pieces, should be a minimum of paddingSize. Since the peers are lost when the header is available, the buffer needs to be big enough to re-initialize the torrent (around 10 should do) (based on bitrate)
    seekPointOffset = 0
        ...
        if not self.parent.parent.canPlay:
            ...
            if pieceAvailableCount >= (self.piecesRequired - self.paddingSize): # x pieces left
                ...
                if not self.forwardBufferRequested:
                    ...
                    self.forwardBufferPieces = self.increaseBuffer(missingPieces=missingPieces, increasePiecePosition=0)
                    self.forwardBufferRequested = True
                else:
                    self.increaseBuffer(missingPieces=missingPieces, increasePiecePosition=self.bufferSize)
